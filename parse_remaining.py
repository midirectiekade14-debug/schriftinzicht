#!/usr/bin/env python3
"""Parse remaining DOCX files for various authors."""
...
_entries = [
    ...
    (["kolossenzen", "kollosenzen", "kol", "kol.", "koloss", "koloss."], "Kolossenzen"),
...
def parse_sermon_docx(path, skip_lines=20):
    """Parse sermons with numbered titles + verse refs."""
...
def parse_verse_commentary(path, skip_lines=5):
    """Parse verse-by-verse commentary (ref + body pattern)."""
    paras = get_paragraphs(path)
    entries = []
    current_ref = None
    body = []
...
def parse_file(path, skip=20):
    """Try sermon detection first, fall back to verse commentary."""
    e = parse_sermon_docx(path, skip)
    if len(e) < 3:
        e = parse_verse_commentary(path, skip)
    return e
...
files_per_author = {
    'kohlbrugge': (14, [
        'kohlbrugge_galaten.docx',
        'kohlbrugge_romeinen.docx',
    ]),
    'dacosta': (16, [
        'dacosta_hagar.docx',
        'dacosta_meijer.docx',
        'dacosta_profetie.docx',
    ]),
    'boston': (12, [
        'boston_4staten.docx',
        'boston_7ental_2.docx',
        'boston_7ental_3.docx',
    ]),
    'luther': (1, [
        'luther_psalm117_118_127.docx',
    ]),
    'brakel': (4, [
        'brakel_rgd.docx',
    ]),
    'calvijn': (2, [
        'calvijn_genesis.docx',
    ]),
    'bunyan': (11, [
        'bunyan_christenreis.docx',
        'bunyan_christinnereis.docx',
    ]),
}
...
for author, (aid, files) in files_per_author.items():
    ...
    if entries:
        out = BASE / f"{author}_remaining.json"
